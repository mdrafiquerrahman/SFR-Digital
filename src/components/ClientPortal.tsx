...
export default function ClientPortal({ initialTab }: ClientPortalProps) {
  ...
  const handleAddTicket = async (e: React.FormEvent) => {
    ...
    const creatorName = portalRole === 'Client' ? (clientRef?.name || 'Client Contact') : 'SFR Digitech Support';
                      ...
                      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                          <h1 className="font-sans text-2xl font-bold tracking-tight">
                            {portalRole === 'Manager' ? 'SFR Digitech Operations Center' : `Welcome Back, ${clients.find(c => c.id === simulatedClientId)?.company || 'Valued Partner'}`}
                          </h1>
